...
def ed_tipo_nom(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_nomina, Descripcion from tipo_nomina where id_nomina= %s', (id))
    dato=cursor.fetchall()
    conn.close()
...
def modifica_tipo_nomina(id):
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('update tipo_nomina set  descripcion=%s where id_nomina=%s', (descrip, id))
        conn.commit()
        conn.close()
    return redirect(url_for('tipo_nomina'))
...
def bo_tipo_nomina(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    #cursor.execute('select count(*) from tipo_nomina where id_tipo_nomina = {0}'.format(id))
    #solicitudes=cursor.fetchone()
    #if (solicitudes[0] != 0):
    #    error = "El nivel tiene dependientes, no puede ser borrado."
     #   return render_template("error.html", des_error=error, paginaant="/agr_datos_nivel_academico")
    #else:
    cursor.execute('delete from tipo_nomina where id_nomina = {0}'.format(id))
    conn.commit()
    conn.close()
    return redirect(url_for('tipo_nomina'))
...
def ed_periodo(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select idperiodo, descripcion from periodo where idperiodo= %s', (id))
    dato=cursor.fetchone()
    conn.close()
...
def modifica_periodo(id):
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('update periodo set  descripcion=%s where idperiodo=%s', (descrip, id))
        conn.commit()
        conn.close()
    return redirect(url_for('periodo'))
...
def percepciones():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select idpercepcion, descripcion from percepcion order by descripcion')
    datos=cursor.fetchall()
    conn.close()
    return render_template("tabla_percepcion.html", percepciones = datos )

#Percepcion nueva
@app.route('/agr_percepcion')
def agr_percepcion():
    return render_template("agrega_percepcion.html")

@app.route('/nueva_percepcion')
def nuevo_percepcion():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    ...
    datos2 = cursor.fetchall()
    conn.close()
    return render_template("agrega_percepcion.html", periodos=datos1, tipos_nom=datos2)
...
def agrega_percepcion():
    if request.method == 'POST':
        ...
        tipo_concepto = request.form['tipo_cpto']
        tipo_nom = request.form['tipo']
        valor = request.form['valor']

        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from percepcion where Descripcion = %s',(concepto))
        per= cursor.fetchone()
        if (per[0] != 0):
            error = "La percepción ya se encuentra agregada."
            return render_template("error.html", des_error=error, paginaant="/nueva_percepcion")
        else:
            cursor.execute('insert into percepcion (descripcion, idperiodo, dia, mes, pornomina_personal, id_nomina, valor) values (%s, %s, %s, %s, %s, %s, %s)', (concepto, periodo, mes, dia, tipo_concepto, tipo_nom, valor ))
            conn.commit()
            conn.close()
